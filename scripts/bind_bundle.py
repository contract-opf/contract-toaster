...
def bind_bundle(
    ...
    *,
    ...
    Raises BindBundleError (never a fabricated/partial bundle) if:
      ...
      - opf_doc's declared identity.content_hash does not VERIFY against its
        own canonical content (see below).
      - review_policy_path is given and the policy is missing/invalid, or its
        playbook_id disagrees with this bundle's.
    ...
    `review_policy_path` (OPF 0.3) is the prescriptive human-authored policy
    (playbooks/<playbook_id>-policy-v<N>.json). None means this playbook has no
    policy -- a legitimate state (corpus knowledge, no prescriptive rules), and
    the bundle then carries no `review_policy` key at all rather than an empty
    one. Not to be confused with `model_policy`, the model/provider config.
    ...
    # Verify, don't trust. lineage.opf_content_hash below is copied from
    # identity verbatim and is what every downstream review records as "the
    # playbook that governed this document". Copying an unverified hash would
    # make that record a claim rather than a fact: an OPF edited after compile
    # would be bound under its stale, honest-looking hash. `load_opf_document`
    # verifies on the upload path, but bind_bundle is also called directly on
    # in-memory docs, so it re-verifies rather than assuming a caller did.
    if not opf_canonicalize.verify_content_hash(opf_doc):
        raise BindBundleError(
            "OPF document's identity.content_hash does not match the hash recomputed "
            "over its own canonical content -- the document changed after it was "
            "compiled. Refusing to bind: the bundle's lineage would record a hash "
            "that does not describe the playbook it ships."
        )
    ...
    if review_policy_path is not None:
        ...
        try:
            policy_doc = policy_load.load_policy(review_policy_path)
        except policy_load.PolicyValidationError as exc:
            raise BindBundleError(f"--policy could not be bound: {exc}") from None
        if policy_doc["playbook_id"] != playbook_id:
            raise BindBundleError(
                f"--policy is for playbook_id {policy_doc['playbook_id']!r} but this bundle "
                f"binds {playbook_id!r}. A policy governs exactly one playbook; binding one "
                "playbook's rules onto another would silently review a contract against the "
                "wrong positions."
            )
        review_policy_block = {
            "path": _repo_relative(review_policy_path),
            "version": policy_doc["version"],
            "hash": policy_load.policy_content_hash(policy_doc),
            "approval_status": policy_doc["approval"]["status"],
        }
    ...
    if review_policy_block is not None:
        bundle_doc["review_policy"] = review_policy_block
    if precision is not None:
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Path to the OPF document to bind: a playbook.opf.html single-file bundle "
            "(the engine's primary distribution artifact) or a bare .opf.json. Either "
            "way its identity.content_hash must verify."
        ),
    )
    parser.add_argument("--model-policy", required=True, help="Path to a model-policy JSON file (model/provider config).")
    parser.add_argument("--playbook-id", required=True, help="Registry key; must be the OPF's own id or alias.")
    parser.add_argument(
        "--policy",
        default=None,
        help=(
            "Path to the review POLICY document (prescriptive human-authored rules). "
            "Defaults to the highest-versioned playbooks/<playbook_id>-policy-v<N>.json. "
            "Distinct from --model-policy."
        ),
    )
    parser.add_argument(
        "--no-policy",
        action="store_true",
        help=(
            "Bind without a review policy. Only legitimate when the playbook genuinely has "
            "no prescriptive rules -- otherwise the review would silently run on corpus "
            "precedent alone, with none of the human-authored positions applied."
        ),
    )
    parser.add_argument(
        "--precision-profile",
    ...
    try:
        # load_opf_document, not load_opf: accepts the .opf.html primary artifact
        # as well as bare .opf.json, and requires+verifies identity on both.
        opf_doc = opf_load.load_opf_document(opf_path)
    ...
    if not args.no_policy:
        if args.policy:
            ...
            review_policy_path = policy_load.resolve_latest_policy_path(args.playbook_id)
            if review_policy_path is None:
                print(
                    f"ERROR: no review policy found for playbook_id {args.playbook_id!r} "
                    f"(looked for playbooks/{args.playbook_id}-policy-v<N>.json). Pass "
                    f"--policy PATH, or --no-policy if this playbook genuinely has no "
                    f"prescriptive rules.",
                    file=sys.stderr,
                )
                return 1
