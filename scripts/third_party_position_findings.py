...
description` and `topics[].our_standard` are tenant-authored content and
may name that tenant's own organization outright). Rationale text uses
"your" voicing and is asserted (in `tests/test_third_party_position_
findings.py`) free of tenant-brand strings -- a stand-in tenant brand name used
to prove no playbook-sourced brand string leaks into user-facing text --
per the project-wide de-brand rule.
...
covering every playbook topic id at least once:

  {
    ...
    "rationale": "<'your'-voiced, tenant-brand-free human-facing text>",
...
model_client.py` (`FakeBedrockClient`), any playbook's JSON document
...
def _hard_rejection_finding(
    ...
    # free of tenant-brand strings (de-brand rule).
    rationale = (
...
def main() -> None:  # pragma: no cover - manual/CLI smoke entry point
    ...
    clauses against the default registered playbook, using a
