...
# Issue #412: the DEFAULT playbook_id is now the bundled sample -- a
# "knowledge" profile entry (playbook_registry.profile(), no
# anchor_map_path/section_config_path: it has no standard-form structure for
# this module's anchor-map machinery to build from at all). Calling
# load_section_config() unconditionally on a knowledge-profile default would
# crash (open(None, ...)) on every `import build_anchor_map` in the process,
# not just a call that actually needs it -- so this module-level back-compat
# default degrades to an empty, valid section config instead, exactly like
# scripts/eval_harness.py's own profile-conditional CLI (issue #288) already
# does for the same "knowledge default" case. A caller that needs a REAL
# section config for a "precision" playbook_id calls load_section_config(id)
# with that id explicitly -- this module-level default is back-compat only.
if _DEFAULT_ENTRY.section_config_path is not None:
    _DEFAULT_SECTION_CONFIG = load_section_config(playbook_registry.DEFAULT_PLAYBOOK_ID)
else:
    _DEFAULT_SECTION_CONFIG = {
        "sections": [],
        "coverage_exempt_rationales": {},
        "coverage_exempt_anchors": [],
        "absent_from_form_anchors": set(),
        "structural_anchors": set(),
        "sub_clause_splits": {},
    }

# Back-compat module-level names (default playbook_id only -- see docstring
# above). Order preserved for stable artifact output.
...
def main():
    ...
    if entry.section_config_path is None:
        print(
            f"ERROR: playbook_id {args.playbook_id!r} is a 'knowledge' profile entry "
            "(playbook_registry.profile()) -- it has no section_config_path, so there "
            "is no standard-form structure to build an anchor map from. Pass "
            "--playbook-id for a 'precision' profile playbook_id instead.",
            file=sys.stderr,
        )
        sys.exit(1)
