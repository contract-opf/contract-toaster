...
## Usage
    ...
    with open("playbooks/samples/synthetic-nda-sample-v1.0.0.json") as f:
        doc = json.load(f)
...
CLI:
    ...
    python3 scripts/canonicalize.py --playbook-id synthetic-generic --record  # write golden-hash fixture
...
def _record_golden_hash(playbook_path: Path, fixture_path: Path) -> None:
    ...
    playbook_path_repo_relative = playbook_path.resolve().relative_to(REPO_ROOT).as_posix()

    fixture = {
        "description": (
            "Golden-hash fixture for CI: records the expected content_hash of the "
            "registered 'eiaa' playbook_id's playbook at a known commit.  If this "
            "hash drifts, tests/test_canonicalize.py::test_golden_hash_fixture "
            "fails, alerting implementers that the canonical form or playbook "
            "content changed. Re-run `python3 scripts/canonicalize.py --playbook-id "
            "eiaa --record` to update after an intentional playbook-content change."
        ),
        "playbook_path": playbook_path_repo_relative,
        "content_hash": h,
