...
- **Manual-review is a system status, not a legal category.** When the pipeline does not complete a review to a legal decision — an oversized document, an unprovable edit, or a structured-output/leakage failure (see [Storage → `status` vs `confidence_state`/`confidence_band`](#storage) for the full, current list) — the UI shows it as a distinct **system status** (`MANUAL_REVIEW_REQUIRED` / `ERROR_MANUAL_REVIEW_REQUIRED`), visually separate from the `ACCEPT | REQUEST_CHANGE` legal decisions, so a reviewer never mistakes a pipeline outcome for a legal opinion. A completed review with a degraded `confidence_band` is **not** this case — see the next bullet: it is still `DONE` (Option B, issue #96).
...
| POST   | `/api/reviews`             | allowlisted   | Upload a `.docx` with an optional `playbook_id` selector (defaults to `eiaa`); idempotency-keyed; returns `202` + review id, starts async pipeline directly. Optional multipart fields: `toaster_guidance` (free text), `notes_mode` (`none`/`external`/`internal`/`both`, default `external`), and `markup_intensity` (issue #54: closed enum `light`/`medium`/`heavy`, default `medium`; any other value is a `400` with `detail` `"markup_intensity must be one of: light, medium, heavy."`, refused before the file is read). `markup_intensity` is recorded on the review row and the submission's execution input only when not `medium`, returned by `GET /api/reviews/{id}` and the list, and rendered by `scripts/primary_review_pass.py::render_markup_intensity_block` as its own fixed-wording system block immediately before the toaster-guidance block on both review paths (none for `medium`) |
...
| GET    | `/api/review-duration-estimate` | allowlisted | How long a review of `?playbook_id=` has actually been taking here (issue #71): `p50_seconds`, `p90_seconds`, `median_words` and `sample_size`, taken over the newest 50 `DONE` rows for that playbook via the `status-index` GSI (never a scan). Measured, never modelled — every figure but `sample_size` is `null` below five usable rows, and a caller that gets `null` renders no estimate at all. Usable means the row carries both `created_at` and the issue-#71 `completed_at`, so a mock-pipeline row (deliberately never stamped) and a still-running review are not samples. Any signed-in user, on the same reasoning as `/api/review-cost-estimate`: the projection is an explicit allowlist (`reviews.REVIEW_DURATION_ESTIMATE_FIELDS`) of aggregates over other reviewers' rows — no review id, no owner, nothing document-derived. `?words=` is the caller's own preflight word count, validated but not consumed: `median_words` is returned instead and the caller does the scaling, which is what keeps one answer true for every reviewer of a playbook. |
...
### LLM — Bedrock + Claude (reviewer and critic passes)

We call Bedrock's `InvokeModel` with Anthropic Claude in `us-east-1` using the **single-region native model ID** (e.g. `anthropic.claude-opus-4-8`), invoked against the regional endpoint — **never a cross-region inference profile**. Per-token rates match the direct Anthropic API. Bedrock's data plane keeps prompts and outputs inside the AWS boundary and contractually disclaims any training use. The two passes are the **reviewer** and the **critic**. Under [ADR 0001](docs/adr/0001-critic-has-the-last-word.md) the critic is the senior reviewer and has the last word on the review, and the model policy requires its tier to be **at least** the reviewer's — the default policy puts the stronger model on the critic. Decorrelating two different models is no longer the reason for the second pass; authority over the output the second pass is best placed to improve is.

#### Model-selection policy

The model is governed by an explicit **model-policy matrix**, not a vague "best model" default. The policy pins:

- **A specific in-region model ID and region for each role.** The release bundle records `primary_model_id`, `critic_model_id`, optional `fallback_model_id`, `model_region`, request contract, evaluation run, and cost assumptions. `embedding_model_id` is **carried but inert** — the field exists in the bundle schema (`playbooks/schema.json`) and shipped sample bundles still populate it (e.g. `playbooks/samples/synthetic-nda-sample-v1.0.0.json`), but nothing embeds, nothing reads it, and no governance obligation attaches to it today. See the bullet below. v1 pins **Opus 4.8 as the reviewer** in `us-east-1`, and [ADR 0001](docs/adr/0001-critic-has-the-last-word.md) makes the critic's tier a policy rule rather than a cost choice: the pinned `critic_model_id` must name a model **at or above** the reviewer's tier, and the default policy puts the stronger model on the critic (see [Two-pass review](#llm--bedrock--claude-reviewer-and-critic-passes)). Re-pinning `critic_model_id` in the model-policy artifacts and warning on a below-tier selection on the Models screen are epic #134's models ticket, not this section — until that lands the shipped artifacts still carry the older, cheaper critic, and that gap is a sequenced decision rather than drift. A later evaluated policy may deliberately change the matrix.
...
| Markup-intensity block (system prompt, issue #54 — `scripts/primary_review_pass.py::render_markup_intensity_block`, fixed wording per level, positioned immediately BEFORE the toaster-guidance block on both review paths: after the standing-instructions block in `primary_review_pass.assemble_system_blocks` (registry-v1), and ahead of the `compose_opf_system_blocks` knowledge blocks in `review_spine._assemble_opf_system_blocks` (OPF) — so the reviewer's own typed words read last; it is an outer control block like toaster guidance, not part of `knowledge.content_hash()`) | **Included for `light` and `heavy`; omitted entirely for `medium`** (the default — a medium review's system prompt is byte-identical to before the block existed, pinned by a golden hash in `tests/test_review_routes_markup_intensity_54.py`). The level comes from `POST /api/reviews`' `markup_intensity` field via the execution input; it never rides in `toaster_guidance` | Same block, same rule — both assemblers are shared by both passes (`review_spine._assemble_opf_system_blocks`; `critic_review_pass.run_critic_pass` -> `primary_review_pass.assemble_system_blocks`) |
...
**Two-pass review — the critic has the last word.** Each review runs two model passes as distinct Step Functions steps. The authority split is fixed by [ADR 0001](docs/adr/0001-critic-has-the-last-word.md):

1. **Reviewer (primary pass)** — produces the first decision and per-issue list as above.
2. **Critic (senior reviewer, final pass)** — using the `critic_model_id` pinned in the active model-policy matrix, it is given the playbook, the standard-form diff, the anchored clause text, the counterparty document the reviewer read, and the reviewer's complete output (issue #618 reversed the original decision to withhold the raw document from the critic — see [Per-pass prompt manifest](#per-pass-prompt-manifest)). It is prompted to read the reviewer's work with a sceptical eye and **emits the final review** — the final issues and the final block-anchored replacement text — rather than a sidecar of objections. Its authority is **validated later-in-time**: it runs later, it has strictly more information, and the model policy requires it to be at least as capable as the reviewer.

That authority is over analysis and drafting only. It never extends to suppressing a hard-rejection ledger entry, every reviewer issue and ledger entry must carry an explicit disposition, and edits are addressed by block id plus original-text hash rather than by character offset — see the ADR's Amendment. The critic schema, prompt and merge work that implements this split are epic #134's later tickets; this section records the decision they implement, and the shipped pipeline still runs the add-only merge until they land.
...
- **Hard rejections and detector fires are monotonic.** Any hard rejection raised by *either* pass, or by a deterministic detector, forces the overall decision to `REQUEST_CHANGE`. One the reviewer or a detector raised and the critic dropped is re-appended. The critic may add rejections; it may not remove one, and it cannot downgrade a hard rejection the reviewer found.
- **Every override is recorded, and the critic's words are the ones that ship.** The earlier add-only rule — the critic could flag the reviewer's `proposed_replacement_text` as drifting but never replace it, so a contested replacement left the reviewer's text in the document — is **superseded by [ADR 0001](docs/adr/0001-critic-has-the-last-word.md)**. For each issue where the critic changed the reviewer's replacement text, rationale or decision, `critic_delta` records the reviewer's version, the critic's version and the critic's stated reason, so nothing the reviewer said is lost from the audit record. (The wire schema still carries the older *contested*-replacement shape; changing it is a later ticket in epic #134 — see [docs/output-contract.md → Critic-delta presentation](docs/output-contract.md#critic-delta-presentation).)
- **The critic's text passes every gate the reviewer's did.** Block-transcript proof against the document bytes, pen rules, the leakage scan on the external channel, and the OOXML round trip all run over the critic's output. A critic failure fails the review; the reviewer's output never ships alone.
...
**Critic-pass failure is terminal — never a silent single-pass DONE.** If the critic invocation fails terminally (after its bounded retry: throttle, outage, or persistently schema-invalid critic output), the review must land in `ERROR` (if the failure is an infrastructure or availability problem) or `ERROR_MANUAL_REVIEW_REQUIRED` (if the critic will not produce schema-valid output after one retry). A critic failure must **never** produce a silent single-pass `DONE` — under [ADR 0001](docs/adr/0001-critic-has-the-last-word.md) the critic authors the final review, so a silent single-pass `DONE` would ship the reviewer's output alone, which the third reconciliation guard above forbids outright. The Step Functions step for the adversarial pass has its own timeout and retry budget; exhausting that budget is a terminal failure of that step, and the pipeline status transitions accordingly. Every critic invocation attempt — including retries and failures — is ledgered in the spend ledger's finally path.
...
**Prompt caching is a within-model optimization, not a cost guarantee.** Caching is enabled on the playbook block (≈30K tokens, changes rarely) and the static review-guidance content. Cache hits are **per-model**: a cache built by the reviewer pass can never serve a critic pass pinned to a different model — each model maintains its own independent cache. Within a single model, a cache hit reduces per-call cost and latency. At v1 production volume (2–7 reviews/day spread across a workday against a ~5-minute TTL), the inter-review steady-state hit rate is **near zero** — the time between reviews is far longer than the cache TTL. Caching therefore delivers savings primarily on **back-to-back retries and eval runs** (where calls are sequential within seconds or minutes), not on typical spaced production usage. The cost model and the daily ceiling are sized to **survive a 0% cache-hit run** — a cache miss makes a review more expensive, never breaks it.
...
Rejected drafts can poison the model if commingled with accepted precedent. Positive precedent and **rejected/negative examples live in separate corpora** (or, equivalently, are partitioned by `corpus_polarity`). Rejected language only ever reaches the model through a **controlled, hard-labeled negative-example channel**; rejected clauses are never placed in the same top-K positive-precedent context. The labels are explicit in the prompt (see [LLM prompt structure](#llm--bedrock--claude-reviewer-and-critic-passes)).
...
- `MANUAL_REVIEW_REQUIRED` — the pipeline did not complete the review to a legal decision (an oversized document, an unprovable edit, or another pre-model/redline fail-closed path — see [`status` vs `confidence_state`/`confidence_band`](#storage) below). Never a consequence of a low `confidence_band` alone on a review that otherwise completed (Option B, issue #96) — that review is `DONE`.
...
**`status` vs `confidence_state`/`confidence_band` (independent since Option B, issue #96).** `confidence_state` is the **internal** signal the pipeline computes (`OK | LOW_CONFIDENCE | MANUAL_REVIEW_REQUIRED | ERROR_MANUAL_REVIEW_REQUIRED`, per [playbooks/schema.json](../playbooks/schema.json) `output_format.system_status`), and `confidence_band` is its non-`OK` mirror surfaced to the reviewer pre-download (see [docs/output-contract.md → Confidence band](docs/output-contract.md#confidence-band)). Before 2026-09-16 this section described `status` as a **terminal projection** of `confidence_state` — a fixed mapping in which `LOW_CONFIDENCE` with no concrete playbook issue produced `status = MANUAL_REVIEW_REQUIRED`, and a `DONE` status paired with a non-`OK`/non-decision `confidence_state` was stated to be invalid. That mapping was never wired: `confidence_state`/`confidence_band` reached neither `scripts/review_spine.py::run_review`'s result nor the `reviews` row until issue #96, so no review's `status` was ever actually derived from it. Owner decision (issue #96, Option B) retires the projection outright rather than implementing it: **a review that completes is `DONE`; only a run that does not complete is a failure.** `confidence_band` (and the critic-delta merge that can degrade it, [docs/output-contract.md → Critic-delta confidence merge rule](docs/output-contract.md#critic-delta-confidence-merge-rule)) is carried onto the result, the analysis artifact, and the row as **pre-download trust-calibration metadata only** — including a `MANUAL_REVIEW_REQUIRED`-or-worse band — and has no bearing on `status`: `status = DONE` whenever the pipeline reached a legal decision (`ACCEPT`/`REQUEST_CHANGE`), and that review delivers its file exactly as any other `DONE` review does. The external legal decision (`ACCEPT | REQUEST_CHANGE`) lives in `decision`; `confidence_band` is a **system status label, never a third legal category and never a terminal-status input**. `status = MANUAL_REVIEW_REQUIRED` / `ERROR_MANUAL_REVIEW_REQUIRED` remain reserved for a run that genuinely did **not** complete to a legal decision: **pre-model pipeline-terminal writes** — a pipeline step that terminates *before any model call*, the sole defined case being the step-14 oversized-document gate (assembled prompt exceeds `max_input_tokens` → `status = MANUAL_REVIEW_REQUIRED` / `reason = document_too_large`, no `confidence_state` produced because the model was never invoked); an unprovable block transcript or an unnormalizable document (`scripts/redline_generate.py`'s own fail-closed paths, `reason` naming which); or `ERROR_MANUAL_REVIEW_REQUIRED` for schema-invalid-after-retry or a leakage-scan hit. **Exception — post-terminal administrative overlays:** `QUARANTINED` and `SUPERSEDED` remain outside this rule entirely, applied by administrative actions (rollback sweeps, manual GC action) *after* the pipeline has already written a terminal status.
...
A counterparty `.docx` can carry pre-existing **tracked changes, comments, hidden text, fields, footnotes, and embedded objects** that would otherwise corrupt both the diff and the redline. Before any review work, the document passes a **normalization pass** that applies a **documented accept/reject rule** to existing revisions and produces a clean canonical body for extraction and diffing. A **pending** (`w:ins`/`w:del`) counterparty revision — the flagship counterparty-markup scenario, and what a redline *is* — is the proposal under review: it is **accepted-all** into the operative draft, with the disposition recorded in a normalization note (never silent); the downstream standard-form diff then recovers what changed. This holds regardless of how many pending clusters or revision authors a paragraph carries — every cluster's resulting text is the same whole-paragraph accept-all text, so there is nothing to silently compose between them; the note names the cluster/author counts when more than one is present. A pending revision inside a field code (a cross-reference, auto-numbering, or date field under live counterparty edit) accepts-all the same way (issue #530): the whole paragraph's accept-all text becomes the operative text exactly as it does for any other pending revision, and the note names what the field itself now resolves to. Those are two different strings whenever the field sits inside a longer sentence, and the extractor carries them as two separate keys (issue #99): the paragraph-level resulting text is what is folded into the operative draft, while the field's own resolved display text is what the note quotes — a date field resolving to `Feb 1` is reported as `Feb 1`, never as the sentence around it. A tracked change that strikes an **entire paragraph** accepts-all the same way (issue #93): its resulting text is the empty string, which is a determinate answer and not a missing one, so the struck paragraph contributes nothing and is **omitted from the operative draft** rather than carried as an empty paragraph, and the note says so at the granularity that is actually true for the caller: `normalize_input.normalize()` calls this once per LOGICAL clause, so there the clause itself is what is omitted; `extraction_normalization_stage.normalize_paragraphs` calls it once per PHYSICAL paragraph while interpolating the clause heading, so there only the struck sibling drops out and any other physical paragraph under the same heading still contributes its own text to the block -- the note never claims the clause was removed when it was not. Fail-closed is reserved for the one structure that remains genuinely ambiguous — a revision record whose resulting-text field is **absent** (as distinct from present and empty), where there is genuinely no text to read — plus structurally corrupt input. Separately from the revision rule, the same pass **screens the extracted text for zero-width and bidirectional control characters** — invisible or order-reversing Unicode a counterparty can plant so that what the model reads is not what the attorney sees. A hit fails the document closed the same way and is **never stripped or repaired**, since a silently sanitized document is one the model and the attorney no longer agree on; the refusal note is counts-only. See [docs/threat-model.md → Prompt-injection resistance for uploaded documents](docs/threat-model.md#prompt-injection-resistance-for-uploaded-documents). Comments never gate normalization by themselves, even alongside a pending revision that is itself accepted-all. Hidden text and field results are stripped/resolved to their literal text. Accepting a paragraph's text this way is only half the story: the same disposition is also **materialized directly into the uploaded `.docx`'s bytes** (every `w:del` removed, every `w:ins` unwrapped, and — since issue #685 — the counterparty's pending **formatting** revisions accepted the same way: each `*PrChange` record dropped so the properties now in the document stand, and a tracked move applied by removing its `w:moveFrom` and unwrapping its `w:moveTo`, with the kinds and counts recorded in the normalization note and any revision kind the transform has no rule for reported there too rather than silently passed through) before block-addressing, edit compilation, and the delivered redline run, so the block map, the compiler, the delivered redline — and the model's own read of the document — all operate on one canonical, already-accepted document; the redline is delivered *on* that accepted document, and the attorney keeps their own original to diff against. **Known limitation (issue #563 follow-up), now disclosed rather than silent (issue #113):** this materialization does not apply the semantics of a tracked **paragraph mark** — `<w:pPr><w:rPr><w:del/></w:rPr></w:pPr>`, Word's record of a counterparty proposing to merge a paragraph with the next one, or the `<w:ins/>` twin proposing to split one in two. The revision markers are stripped, but the `<w:p>` elements are neither merged nor divided, so the materialized bytes still show the paragraph structure the counterparty proposed to change; `tests/test_accept_all_materializer.py` pins that as the current, documented behavior. What issue #113 changed is that the proposal is no longer accepted in silence — the marker wraps no run, so extraction built no revision cluster for it and the attorney was told nothing at all. It is now reported in both disclosure channels: a per-paragraph normalization note naming the proposed merge or split, its author, and the fact that it is not applied (`paragraph_mark`, the one disposition in `scripts/normalize_input.py`'s rule that is disclose-only and changes no text), and its own byte-space sentence in the same normalization note, from the paragraph-mark counts the accept-all report now tallies separately from ordinary content `w:ins`/`w:del`. `tests/test_paragraph_mark_disclosure_113.py` pins both channels, and pins that disclosing the proposal moves no operative text, no block id, and no `<w:p>`. If a document cannot be normalized to a clean, unambiguous body, the review **fails closed** to an internal analysis report rather than guessing: the pipeline sets `status = MANUAL_REVIEW_REQUIRED` with `reason = unnormalizable_input` and stores the analysis report in the `outputs` bucket (accessible to owner-or-admin only) instead of a redline `.docx`. The report describes the normalization problem and any intended changes so the attorney can apply them by hand. See [docs/output-contract.md → Fail-closed internal analysis report](docs/output-contract.md#fail-closed-internal-analysis-report) for the format, delivery surface, reviewer-facing copy, and retention classification, and `scripts/normalize_input.py` for the full documented accept/reject rule and `scripts/extraction_normalization_stage.py::materialize_accept_all` for the byte-level transform. (The hostile-file model — zip-bomb, XML-entity, external-relationship, macro-template, MIME/magic-number, AV scan — is owned by [docs/threat-model.md](docs/threat-model.md).)
...
| Content-control placeholders (`w:sdt` with display-only content, i.e. `w:sdtPr/w:showingPlcHdr`) | **Excluded** — stripped outside the extraction allowlist |
| Content controls with actual (filled-in) content, and smart tags (`w:sdt`/`w:sdtContent` NOT showing placeholder text; `w:smartTag`) | **Allowed** — transparent wrapper, same treatment as `w:hyperlink`/`w:moveTo` (issue #94). Transparent at every level the walkers reach: inline (around runs), block (around whole paragraphs), and a table's row/cell level (`w:tbl`→`w:sdt`→`w:tr`, `w:tr`→`w:sdt`→`w:tc`, the shapes Word's repeating-section and table controls emit) |

**Writer-side limitation of the placeholder exclusion (issue #94).** The exclusion above is extraction-only. `redline_block_apply._accepted_text_runs` mirrors `docx_editor.xml_editor.build_text_map`, which keeps an unfilled control's display copy, and that mirror is what makes an offset mean the same span to the library that actually writes the tracked change. So for a paragraph carrying an **unfilled** control the extractor's clean text and the live accepted view differ on purpose, the paragraph does not resolve, and the edits touching **it** fail closed one at a time as `paragraph_not_resolved` — every other edit in the batch still lands. Copying the exclusion into the writer instead would make both sides agree about the text while disagreeing with `docx_editor` about the coordinates, which writes the tracked change into the empty control.
...
Pipeline (Step Functions), asynchronous — status flips to RUNNING:
 ...
 16. Adversarial review: critic_model_id from the active model policy critiques
     ...
     forces REQUEST_CHANGE; under ADR 0001 the critic's text is the text that
     ships and every override is recorded; keep deltas).
 ...
 18. Determine the external decision (ACCEPT | REQUEST_CHANGE) and the internal
     confidence state, mirrored as `confidence_band` whenever that state is not
     OK. The band never projects onto the review's terminal `status` (Option B,
     owner decision on issue #96): a review that reaches a legal decision here
     lands DONE however degraded its band is, and delivers its file like any
     other DONE review. The band is pre-download trust-calibration metadata — a
     system status, not a legal category. See
     [`status` vs `confidence_state`/`confidence_band`](#storage).
...
End-to-end latency: 1–3 minutes typical, 5 minutes p95 (including the standard-form diff and the adversarial pass), depending on document length. The reviewer pass emits 4–8K output tokens, which takes 1.5–4 minutes at typical Opus 4.8 throughput; the critic pass adds further time, and under [ADR 0001](docs/adr/0001-critic-has-the-last-word.md) its output is sized like the reviewer's rather than a short critique, because it authors the final replacement text. A "20–90 seconds" figure predates the full two-pass pipeline and is superseded by this baseline.
...
- **Bedrock (reviewer pass + critic pass).** Dominant *marginal* cost, but $0 when idle (single-region native ID, Standard tier — no provisioned hourly charge). Opus is ~$5.50/M input, ~$27.50/M output (including the ~10% regional endpoint surcharge; direct API base rate is ~$5/M input, ~$25/M output); the cheaper Sonnet 4.6 model the older matrix put in that role was ~$3.30/M input, ~$16.50/M output, but [ADR 0001](docs/adr/0001-critic-has-the-last-word.md) pins the critic at or above the reviewer's tier and makes it the author of the final replacement text, so the second pass must now be planned at the reviewer's own rate with an output as large as the reviewer's — sizing the token reservation for that is epic #134's cost ticket. Prompt caching on the ≈30K-token playbook is a **within-model optimization only** — caches are per-model (two differently pinned roles have separate caches), and at v1 production volume the steady-state inter-review hit rate is near zero (see the cost-shape table above). Caching delivers savings on back-to-back retries and eval runs; the cost model and the reservation are sized to survive a 0% cache-hit run. Note: the current Opus tokenizer produces noticeably more tokens than the prior generation for the same text, so real cost runs higher than naïve estimates — bounded by the **atomically-reserved $20/day ceiling** (an atomic reservation priced from the caps, including every retry attempt, and reconciled against the ledger at settlement — not an upper bound on settle; see [Backend idempotency](#api-vs-async-worker) and the caps table above), and **every model attempt (including failures and retries) is ledgered** so the spend record is complete.
