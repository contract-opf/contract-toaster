...
| Full counterparty document text | **Included only if doc token count ≤ `full_doc_token_threshold` (default: 60,000 tokens, issue #419)**; above threshold, replaced by section outline (heading + word count per section), `input_mode="section_outline"` on the review result, `confidence_state` degraded one level, and a fixed notice appended to the summary | **Not included** — the critic does not receive the raw counterparty document; the diff + anchored clauses already encode all counterparty changes, and omitting the full doc from the critic removes the primary redundancy identified in the architecture review 2026-06-11 |
