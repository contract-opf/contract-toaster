...
def main() -> int:
    if not CODEOWNERS_PATH.exists():
        # The public cut deliberately ships no CODEOWNERS (issue #406): it
        # routes review to teams in the private origin org, which do not exist
        # in the public org, so the file would be both non-functional there and
        # the most visible place the origin org name appears. This check gates
        # review ownership in the repo that HAS owners; where the file is
        # absent by design there is nothing to gate, so skip rather than fail
        # and turn the published mirror's CI red.
        print(f"SKIP: {CODEOWNERS_PATH} does not exist "
              "(excluded from the public cut — nothing to gate here).")
        return 0
