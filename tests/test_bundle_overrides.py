...
# OPF 0.3 gold fixture. The #294 override semantics under test here are
# version-independent, but prompt composition now requires a digest (an OPF 0.2
# document has none, and composing knowledge-free would review a contract on
# model judgement alone). Production binds 0.3, so compose against 0.3.
FIXTURE_03_PATH = REPO_ROOT / "tests" / "gold-fixtures-opf" / "acme-university.opf.json"
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        result = subprocess.run(
            [
                ...
                "--playbook-id",
                "eiaa",
                # eiaa carries no committed review policy (issue #413 evicted
                # the real tenant-derived harvest); this check exercises the
                # stale-edit guard, not policy resolution.
                "--no-policy",
                "--posture-override",
                str(override_path),
                "--out",
...
def _load_03_fixture() -> dict:
    with open(FIXTURE_03_PATH, encoding="utf-8") as f:
        return json.load(f)


def check_6_composition_posture_and_floor_union() -> list[str]:
    failures = []
    opf_doc = _load_03_fixture()
    ...
    # Found by its intro, not its index: PR F1 fixed the order at POSTURE,
    # BINDING, DIGEST, ... and omits a block entirely when it has nothing to
    # say, so an index reads whatever moved into the slot.
    floor_block = next(b for b in override_blocks if opf_prompt.BINDING_INTRO in b)
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        result = subprocess.run(
            [
                ...
                # eiaa carries no committed review policy (issue #413 evicted
                # the real tenant-derived harvest); this check exercises the
                # overrides/CLI plumbing, not policy resolution.
                "--no-policy",
                "--posture-override",
                str(posture_path),
                "--floor-additions",
