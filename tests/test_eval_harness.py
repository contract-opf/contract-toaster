...
def _make_case(raw: dict) -> "eval_harness.GoldCase":  # noqa: UP037
...
def check_clause_text_with_xml_metacharacters_does_not_raise() -> list[str]:
    ...
    docx_bytes, shown_paragraphs = eval_harness.build_document(case["document"]["clauses"])  # noqa: RUF059
...
def check_generated_fixtures_are_synthetic() -> list[str]:
    ...
    for fixture_path in sorted(GOLD_FIXTURES_DIR.glob("*.json")):
        with open(fixture_path, encoding="utf-8") as f:
            fixture = json.load(f)
        if fixture.get("generated_by") == "scripts/generate_gold_fixtures.py":  # noqa: SIM102
