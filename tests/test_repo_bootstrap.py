...
bootstrap (issue #49) are satisfied:
  ...
  D. playbooks/schema.json and playbooks/samples/synthetic-nda-sample-v1.0.0.json
     are committed.
...
def check_b_github_structure() -> list[str]:
    ...
    # CODEOWNERS is deliberately absent from the public cut (issue #406) --
    # it routes review to teams in the private origin org that do not exist in
    # the public org. Required only where review ownership is actually gated.
    required = [
    ...
    if (github / "CODEOWNERS").exists():
        required.insert(0, ("CODEOWNERS", github / "CODEOWNERS"))
...
def check_d_playbooks() -> list[str]:
    print("\nCheck D: playbooks/ artefacts …")
    failures = []
    # issue #403: the engine ships no baked-in tenant playbook. issue #412:
    # the committed, brand-free artifact proving playbooks/ is a working
    # profile is now the bundled Synthetic NDA Sample -- "sample-agreement"
    # was removed entirely (it was never the shipped sample).
    for rel_path in ("schema.json", "samples/synthetic-nda-sample-v1.0.0.json"):
        path = REPO_ROOT / "playbooks" / rel_path
        failures += _assert(
            path.is_file(),
            f"playbooks/{rel_path} exists",
            f"Expected: {path}",
        )
    return failures
...
def check_g_codeowners_eval() -> list[str]:
    print("\nCheck G: CODEOWNERS gates eval/** under the GC team …")
    path = REPO_ROOT / ".github" / "CODEOWNERS"
    if not path.is_file():
        # Absent by design in the public cut (issue #406) -- see check B.
        print("  SKIP: no CODEOWNERS in this tree (excluded from the public cut).")
        return []
    failures: list[str] = []
