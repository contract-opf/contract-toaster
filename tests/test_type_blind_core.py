...
Covers ACs 1-2 of issue #289:
  ...
  AC2: an unregistered playbook_id on the Docker Compose mock path (real moto
...
class TestRegistryOwnedDefault(unittest.TestCase):
    ...
    def test_real_registry_default_is_the_bundled_sample(self):
        """Guard: the REAL playbooks/registry.json carries
        "default_playbook_id": "synthetic-nda-sample" (issue #412 -- the
        toaster ships and loads exactly one playbook, the bundled Synthetic
        NDA Sample; the renamed "synthetic-generic" entry -- née "eiaa" --
        remains separately registered and loadable for the test suite, but
        is marked test_only and is never the default)."""
        self.assertEqual(playbook_registry.default_playbook_id(), "synthetic-nda-sample")
...
class TestCorpusResolvesDefaultViaRegistry(unittest.TestCase):
    ...
    def test_real_corpus_default_is_the_bundled_sample(self):
        """Guard: the real registry default is now "synthetic-nda-sample"
        (issue #412)."""
        record = corpus.build_clause_record(
            source_document_id="doc-1",
            playbook_topic_id="t1",
        ...
        self.assertEqual(record["playbook_id"], "synthetic-nda-sample")
...
class TestMockPipelineIsRegistryDriven(unittest.TestCase):
    ...
    def test_registered_without_mock_output_key_is_manual_review(self):
        ...
        "synthetic-nda-sample" entry, issue #412's bundled sample -- a
        knowledge-profile playbook with no pre-baked mock redline) gets the
        "playbook coming soon" copy, not the generic unknown-playbook copy
        -- the two MANUAL_REVIEW_REQUIRED branches are distinguishable."""
        pr.run_mock_pipeline(
            self.REVIEW_ID,
            {"review_id": self.REVIEW_ID, "playbook_id": "synthetic-nda-sample"},
            dynamodb_resource=self.ddb,
            s3_client=self.s3,
        )
        item = self.reviews_table.get_item(Key={"review_id": self.REVIEW_ID})["Item"]
    ...
    def test_eiaa_still_reaches_done_with_pre_baked_fixture(self):
        """Guard: behavior identical for the renamed "synthetic-generic"
        entry -- née "eiaa" (issue #289's acceptance criteria, issue #412's
        rename) -- even though _mock_decision no longer compares against
        any playbook_id literal -- it now reads registry.json's
        mock_output_key."""
        entry = playbook_registry.resolve_playbook("synthetic-generic")
        self.assertIsNotNone(
            entry.mock_output_key,
            "playbooks/registry.json's synthetic-generic entry must carry "
            "mock_output_key (issue #289 spot 4).",
        )
        self.s3.put_object(Bucket=OUTPUTS_BUCKET, Key=entry.mock_output_key, Body=b"pre-baked bytes")

        pr.run_mock_pipeline(
            self.REVIEW_ID,
            {"review_id": self.REVIEW_ID, "playbook_id": "synthetic-generic"},
            dynamodb_resource=self.ddb,
            s3_client=self.s3,
        )
        item = self.reviews_table.get_item(Key={"review_id": self.REVIEW_ID})["Item"]
...
class TestReviewRoutesDefaultPlaybookIdFromRegistry(unittest.TestCase):
    ...
    def test_real_default_is_the_bundled_sample(self):
        """Guard: the real registry default is now "synthetic-nda-sample"
        (issue #412 -- the toaster ships and loads exactly one playbook)."""
        self.assertEqual(review_routes.DEFAULT_PLAYBOOK_ID, "synthetic-nda-sample")
