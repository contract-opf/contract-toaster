...
class TestRequireActiveUser(unittest.TestCase):
    def test_active_user_is_allowed_and_row_returned(self):
        ...
        _seed_user(users, "sub-1", "reviewer@example.com", status_="active")
    ...
    def test_suspended_user_denied_403(self):
        ddb, users, _, _ = _new_ddb()
        _seed_user(users, "sub-2", "suspended@example.com", status_="suspended")
    ...
    def test_deprovisioned_user_denied_403(self):
        ddb, users, _, _ = _new_ddb()
        _seed_user(users, "sub-3", "gone@example.com", status_="deprovisioned")
        with self.assertRaises(HTTPException) as ctx:
...
class TestListUsers(unittest.TestCase):
    def test_non_admin_403(self):
        ddb, users, _, _ = _new_ddb()
        _seed_user(users, "sub-1", "reviewer@example.com", is_admin=False)
    ...
    def test_admin_sees_all_users_including_jit_created_rows(self):
        ddb, users, _, _ = _new_ddb()
        _seed_user(users, "admin-1", "admin@example.com", is_admin=True, last_auth_at=5000)
        _seed_user(users, "sub-jit", "newreviewer@example.com", is_admin=False, last_auth_at=4000)
        _seed_user(users, "sub-gone", "left@example.com", status_="deprovisioned", last_auth_at=3000)
    ...
    def test_list_is_ordered_most_recent_auth_first(self):
        ddb, users, _, _ = _new_ddb()
        _seed_user(users, "admin-1", "admin@example.com", is_admin=True, last_auth_at=100)
        _seed_user(users, "sub-a", "a@example.com", last_auth_at=50)
        _seed_user(users, "sub-b", "b@example.com", last_auth_at=200)
        caller = _users_module.require_active_user("admin-1", ddb)
...
class TestUpdateUser(unittest.TestCase):
    def setUp(self):
        ...
        _seed_user(self.users, "sub-1", "reviewer@example.com", is_admin=False)
    ...
    def test_unknown_field_rejected_400(self):
        with self.assertRaises(HTTPException) as ctx:
            _users_module.update_user("sub-1", {"email": "new@example.com"}, self.admin, self.ddb)
        self.assertEqual(ctx.exception.status_code, 400)
...
class TestGetUser(unittest.TestCase):
    ...
    def test_admin_can_fetch_a_user(self):
        ...
        self.assertEqual(result["email"], "reviewer@example.com")
