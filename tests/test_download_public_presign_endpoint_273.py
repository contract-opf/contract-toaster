#!/usr/bin/env python3
"""
Unit tests for issue #273: Docker Compose presigned-download URLs must not require a
manual `/etc/hosts` entry (`127.0.0.1 minio`) for a browser on the host to
resolve them.

## Root problem this proves fixed

The Docker Compose target's backend reaches MinIO at `S3_ENDPOINT_URL=http://minio:9000`
...
and `backend/src/config.py` directly and proves:
  ...
  1. AWS target / Docker Compose without the new var: `S3_PUBLIC_ENDPOINT_URL` unset ->
     ...
     attempted.
  2. Docker Compose target: `S3_PUBLIC_ENDPOINT_URL=http://localhost:9000` set, while
...
def _internal_s3_client() -> "boto3.client":
    """The client review_routes.get_s3_client() would build inside the Docker Compose
...
class TestConfigPublicEndpointSeam(unittest.TestCase):
    ...
    def test_presigning_kwargs_override_endpoint_only(self) -> None:
        """Docker Compose path: S3_ENDPOINT_URL (internal) and S3_PUBLIC_ENDPOINT_URL
...
class TestPresignedUrlHostSwap(unittest.TestCase):
    def test_unset_public_endpoint_reuses_injected_client_unchanged(self) -> None:
        """AWS target (and any Docker Compose deployment that hasn't set the new var):
    ...
    def test_public_endpoint_set_swaps_the_presigned_host(self) -> None:
        """Docker Compose target: the injected s3_client is signed for the internal
