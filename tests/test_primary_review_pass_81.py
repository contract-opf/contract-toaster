...
def test_system_blocks_order_and_cache_breakpoint(failures: list[str]) -> None:
    # Issue #398: assemble_system_blocks now appends a judged-NL Floor block
    # whenever the playbook carries hard_rejections (the real eiaa playbook
    # this test loads always does), so the block list is no longer a fixed
    # length of 3 -- match blocks by IDENTITY (their own text), never by a
    # hardcoded index (the same "position is not identity" lesson
    # scripts/opf_prompt.py's own module docstring already documents for
    # its sibling composer).
    playbook = _sample_playbook()
    blocks = pp.assemble_system_blocks(playbook)

    if len(blocks) < 3:
        failures.append(f"[1a] Expected at least 3 system blocks (guidance, overlay, playbook); got {len(blocks)}")
        return

    guidance = blocks[0]
    playbook_block = blocks[-1]

    if guidance["text"] != pp.REVIEW_GUIDANCE_BLOCK:
        failures.append("[1b] System block 0 must be the review-guidance block, verbatim.")
    overlay_matches = [b for b in blocks if b["text"] == pp.BINARY_DECISION_OVERLAY_BLOCK]
    if len(overlay_matches) != 1:
        failures.append(f"[1c] Expected exactly one binary-decision-overlay block, verbatim; found {len(overlay_matches)}.")
    ...
    # inclusion assertions; here we just check the LAST block round-trips to
    # the projected view, not the full input playbook.
    if json.loads(playbook_block["text"]) != pp.project_playbook_for_prompt(playbook):
        failures.append("[1d] The LAST system block must be the PROJECTED playbook JSON, verbatim (round-trips to project_playbook_for_prompt(playbook), issue #267).")

    # Cache breakpoint AFTER the playbook block only (issue #30), which is
    # always the LAST block regardless of how many conditional blocks
    # (toaster guidance, Floor) preceded it.
    if any("cache_control" in b for b in blocks[:-1]):
        failures.append("[1e] Only the LAST (playbook) block may carry cache_control -- no earlier block may.")
...
# ---------------------------------------------------------------------------
# Issue #378: the code-composed BINARY_DECISION_OVERLAY_BLOCK (not any
# playbook file) instructs the model to emit a verbatim `source_quote` per
# REQUEST_CHANGE issue, with `source_quote` added to the overlay's
# authoritative per-issue key set. output-schema-v2.json already accepts the
# field (issue #376) -- this slice is the prompt instruction that actually
# asks the model to populate it. Critic pass inherits automatically because
# it calls assemble_system_blocks() directly (see
...
def test_playbook_projection_excludes_governance_metadata(failures: list[str]) -> None:
    ...
    # The GC approval memo (synthetic-generic-v1.0.0.json -> playbook.legal_approval.note)
    ...
    # not evidence of leakage -- absence of the key is.) The playbook block
    # is always LAST (issue #398 may insert a Floor block ahead of it --
    # see test_system_blocks_order_and_cache_breakpoint's "position is not
    # identity" note), never a fixed index.
...
def test_playbook_projection_includes_all_knowledge_fields_verbatim(failures: list[str]) -> None:
    playbook = _sample_playbook()
    system_blocks = pp.assemble_system_blocks(playbook)
    # The playbook block is always LAST -- see test_system_blocks_order_and_
    # cache_breakpoint's "position is not identity" note (issue #398).
    playbook_block = system_blocks[-1]
    projected = json.loads(playbook_block["text"])
...
def test_replacement_text_violation_then_clean_retries_and_succeeds(failures: list[str]) -> None:
    ...
    # (synthetic-generic-v1.0.0.json) -- a schema-VALID response whose proposed
...
def test_replacement_text_violation_on_final_attempt_demotes_to_flag_only(failures: list[str]) -> None:
    # limitation-of-liability's max_chars is 1200 (synthetic-generic-v1.0.0.json).
