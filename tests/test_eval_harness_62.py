...
buildable BEFORE the LLM pipeline stages (#80-#83) exist:
  ...
  5. A mechanically-derived subset of the gold set: one synthetic planted-
     violation case per existing hard_rejection rule in the playbook
     backing tests/gold-fixtures/ (scripts/generate_gold_fixtures.py).
...
def check_harness_scores_all_fixtures_pass() -> list[str]:
    failures = []
    # Explicit fixtures_dir/playbook_path (issue #412 repointed the registry
    # DEFAULT to the bundled "synthetic-nda-sample" sample playbook -- a
    # knowledge-profile entry with no detector-scorable gold fixtures at
    # all -- so eval_harness.score_all()'s own module-level default no
    # longer resolves to this file's target, "synthetic-generic"/
    # tests/gold-fixtures/, the ~30-fixture set #62 actually gates).
    results = eval_harness.score_all(fixtures_dir=FIXTURES_PATH, playbook_path=PLAYBOOK_PATH)
    if not results:
...
def check_every_hard_rejection_rule_has_gold_coverage() -> list[str]:
    failures = []
    # Explicit fixtures_dir/playbook_path -- same reason as check 2 above.
    missing = eval_harness.missing_rule_coverage(fixtures_dir=FIXTURES_PATH, playbook_path=PLAYBOOK_PATH)
    if missing:
        failures.append(
