...
# A dedicated fixture (issue #410), deliberately separate from
# tests/gold-fixtures/canonicalize-golden-hash.json: that file records the
# golden hash of the registered "eiaa" playbook_id's playbook (issue #403:
# the real eiaa playbook is evicted; "eiaa" now resolves to this SAME
# synthetic fixture, by a different path) and is still relied on by
# tests/test_active_bundle_resolver_194.py (explicitly pinned to "eiaa"),
# so it must not be repointed at this file's own path.
GOLDEN_HASH_FIXTURE_PATH = REPO_ROOT / "tests" / "fixtures" / "playbooks" / "synthetic-generic-golden-hash.json"
...
def test_golden_hash_fixture():
    """
    The golden-hash fixture at
    tests/fixtures/playbooks/synthetic-generic-golden-hash.json records the
    expected content_hash of the synthetic-generic-v1.0.0.json playbook
    fixture at a known commit.  This test fails if the hash drifts,
    alerting implementers that the canonical form or the fixture content
    changed.
    ...
    assert actual_hash == expected_hash, (
        f"content_hash of synthetic-generic-v1.0.0.json HAS DRIFTED from the golden fixture!\n"
