...
def check_arch_sync_deprovisions_only() -> list[str]:
    ...
    if not deprov_on_removal.search(combined):
        failures.append(
            ...
            "legal-admin@teamexos.com group is deprovisioned on next sync."
        )
...
def check_runbook_onboarding_reviewer() -> list[str]:
    ...
    if not section_body:
        failures.append(
            ...
            "The procedure must exist with: (1) add to legal-admin@teamexos.com "
    ...
    if not add_to_group.search(section_body):
        failures.append(
            ...
            "to the legal-admin@teamexos.com group as step 1."
        )
...
def check_runbook_adding_new_admin_group_step() -> list[str]:
    ...
    if not add_to_group.search(section_body):
        failures.append(
            ...
            "step (add to legal-admin@teamexos.com BEFORE signing in).  Without this, "
