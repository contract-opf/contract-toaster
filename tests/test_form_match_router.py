...
router:
  ...
  6. Every router-emitted user-facing string (`MSG_THIRD_PARTY_ROUTE`,
     ...
     tenant-brand strings and uses "your" voicing.
...
def _load_standard():
    return diff_standard_form.load_standard_form_paragraphs(playbook_id="synthetic-generic")
...
def test_router_strings_are_debranded(failures, mod, standard):
    ...
    for s in static_strings:
        for bad in banned:
            if bad.lower() in s.lower() and bad.lower() == "exos":
                failures.append(f"[6a] Router string contains tenant-brand strings branding: {s!r}")
                break
    ...
    for msg in all_messages:
        if msg and "exos" in msg.lower():
            failures.append(f"[6c] Router-emitted user_message contains tenant-brand strings branding: {msg!r}")
