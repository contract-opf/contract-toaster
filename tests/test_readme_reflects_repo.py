...
still leads with a `cdk deploy` AWS flow instead of the Docker Compose
stack described in docs/REVIEW-GUIDE.md.
  ...
  E. README's local-development instructions lead with the Docker Compose
     quickstart (linking docs/REVIEW-GUIDE.md) before the AWS CDK
     deploy flow.
...
def check_ab_layout_matches_tree() -> list[str]:
    ...
    failures += _assert(
        ...
        "The tree has standard-forms/synthetic-generic-v1.0.0.docx; describe "
        "that reality instead of a phantom pending file.",
    )
...
# ---------------------------------------------------------------------------
# Check E — Docker Compose quickstart is primary, before the AWS CDK flow
# ---------------------------------------------------------------------------

def check_e_dts_quickstart_primary() -> list[str]:
    print("\nCheck E: Docker Compose quickstart precedes the AWS CDK deploy flow …")
    text = _read(README)
    failures = []
    ...
    failures += _assert(
        ...
        "the 'docker compose' quickstart appears before the AWS 'cdk deploy' flow",
