...
consequences:
  ...
  2. Anchors registered in the map but never actually present in a real
     draft -- sec-2.2.1 "Without Cause" (deliberately absent from the
...
def main():
    ...
    # Explicit "synthetic-generic" (issue #343 repointed the registry default to the
    # public "sample-agreement" sample playbook) -- this gate exercises
    # synthetic-generic-specific anchors (sec-2.2.1 absent-from-form, sec-preamble/
    # sec-signature structural, sec-8 renamed).
    standard = mod.load_standard_form_paragraphs(playbook_id="synthetic-generic")
