...
standard-forms/README.md admitted the canonical standard-form .docx was "to be
...
Per the 2026-07-10 owner decision (issue #200), the real standard-form text
is a GC deliverable engineering cannot fabricate. This test instead drives
the REAL loader / anchor-map-builder / diff code over a clearly-labeled
SYNTHETIC placeholder — standard-forms/synthetic-generic-v1.0.0.docx (see
...
SYNTHETIC_DOCX = REPO_ROOT / "standard-forms" / "synthetic-generic-v1.0.0.docx"
...
def main():
    ...
    # -------------------------------------------------------------------
    docx_standard = dsf.load_standard_form_paragraphs(docx_path=SYNTHETIC_DOCX, playbook_id="synthetic-generic")
    synthetic_standard = dsf.load_standard_form_paragraphs(playbook_id="synthetic-generic")
    ...
    # synthetic mode reads the registered "synthetic-generic" playbook_id's our_standard
    ...
    # Explicit "synthetic-generic" config (issue #343 repointed the registry default to
    # the small public "sample-agreement" sample playbook, which has no
    # sec-10 sub-clause splits at all).
    eiaa_sections = bam.load_section_config("synthetic-generic")["sections"]
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        anchor_map_data = dsf._load_active_anchor_map("synthetic-generic")
