...
def check_d_codebuild_owner_repo_from_context() -> list[str]:
    ...
    failures += _assert(
        not any("exos-legal/contract-toaster" in s for s in sources),
        "CodeBuild source no longer hard-codes exos-legal/contract-toaster under custom context",
        f"sources found: {sources}",
    )
