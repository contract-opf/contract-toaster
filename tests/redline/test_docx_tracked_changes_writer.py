...
def main() -> None:
    ...
    # =========================================================================
    # Part E — include_marker=False produces NO marker in any part (issue
    # #513 AC2, standalone-writer half). The live first-party path
    # (scripts/redline_generate.py) is asserted elsewhere
    # (tests/redline/test_redline_generation_83.py,
    # tests/redline/test_inplace_tracked_changes.py) but nothing previously
    # exercised `include_marker=False` on THIS module's own writer directly
    # -- `grep -rn include_marker` found no such call anywhere in the repo.
    # This asserts it on generated bytes, closing that gap.
    # =========================================================================

    no_marker_docx_bytes = redline_docx_writer.build_tracked_changes_docx(
        applied_patches, current_paragraphs_by_anchor, include_marker=False
    )

    no_marker_buf = io.BytesIO(bytes(no_marker_docx_bytes))
    if not zipfile.is_zipfile(no_marker_buf):
        failures.append(
            "[E1] include_marker=False output is not a valid ZIP archive."
        )
    else:
        with zipfile.ZipFile(no_marker_buf) as zf:
            no_marker_names = zf.namelist()

            if "word/header1.xml" in no_marker_names:
                failures.append(
                    "[E2] include_marker=False output still contains "
                    "word/header1.xml -- the marker header must be absent "
                    "entirely, not merely empty."
                )
            if "word/footer1.xml" in no_marker_names:
                failures.append(
                    "[E3] include_marker=False output still contains "
                    "word/footer1.xml -- the marker footer must be absent "
                    "entirely, not merely empty."
                )

            no_marker_document_xml = zf.read("word/document.xml")
            no_marker_document_text = no_marker_document_xml.decode("utf-8")

            if redline_docx_writer.MARKER_TEXT in no_marker_document_text:
                failures.append(
                    "[E4] include_marker=False output's word/document.xml "
                    f"still contains the marker text "
                    f"{redline_docx_writer.MARKER_TEXT!r} (e.g. as a "
                    "first-page cover note)."
                )

            no_marker_root = ET.fromstring(no_marker_document_xml)
            header_refs = no_marker_root.findall(f".//{_qn('headerReference')}")
            footer_refs = no_marker_root.findall(f".//{_qn('footerReference')}")
            if header_refs:
                failures.append(
                    "[E5] include_marker=False output's <w:sectPr> still "
                    "carries a <w:headerReference> -- no header should be "
                    "wired when there is no marker to render."
                )
            if footer_refs:
                failures.append(
                    "[E6] include_marker=False output's <w:sectPr> still "
                    "carries a <w:footerReference> -- no footer should be "
                    "wired when there is no marker to render."
                )

            # No part of the archive -- not just document.xml -- may carry
            # the marker text (a stray copy in .rels or elsewhere would
            # still be a leak).
            for name in no_marker_names:
                if name == "word/document.xml":
                    continue  # already checked above with a clearer message
                part_bytes = zf.read(name)
                if redline_docx_writer.MARKER_TEXT.encode("utf-8") in part_bytes:
                    failures.append(
                        f"[E7] include_marker=False output part '{name}' "
                        f"still contains the marker text "
                        f"{redline_docx_writer.MARKER_TEXT!r}."
                    )
