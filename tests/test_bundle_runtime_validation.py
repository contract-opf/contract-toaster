...
# default to the bundled "synthetic-nda-sample" sample playbook) because
# this file's fixtures below assert synthetic-generic-specific anchors (e.g.
# "sec-2.1", the term-length topic's anchor).
REAL_PLAYBOOK_ID = "synthetic-generic"
...
class TestValidBundleUnaffected(unittest.TestCase):
    ...
    # test_real_eiaa_paragraphs_still_load_unchanged deleted (issue #403,
    # obsolete-by-design): its whole purpose was proving the REAL eiaa
    # playbook's paragraph text loads unchanged. The real eiaa playbook is
    # intentionally evicted (playbook_path now resolves to a brand-neutral
    # synthetic fixture, not real tenant content) -- a test named and
    # documented as asserting the real content is no longer meaningful, even
