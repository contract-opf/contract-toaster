...
def check_section_config_fixture(mod):
    ...
    Explicitly resolved for "synthetic-generic" (issue #412 repointed the
    registry default to the bundled "synthetic-nda-sample" sample playbook,
    a "knowledge" profile entry with no section config at all) -- this
    gate's whole point is the sec-10 hand-split config, which only
    synthetic-generic's section-config data file carries."""
    failures = []
    eiaa_section_config = mod.load_section_config("synthetic-generic")
...
def check_docx_mode():
    ...
    # Explicitly "synthetic-generic" (issue #412 repointed the registry
    # default to the bundled knowledge-profile "synthetic-nda-sample" sample
    # playbook) -- see check_section_config_fixture's docstring above.
    eiaa_sections = mod.load_section_config("synthetic-generic")["sections"]
...
def check_deterministic_artifact():
    ...
    Explicitly "--playbook-id synthetic-generic" (issue #412 repointed the
    registry DEFAULT to the bundled "synthetic-nda-sample" sample playbook,
    a "knowledge" profile entry the anchor-map builder refuses outright --
    see build_anchor_map.main()'s own guard): this gate is inherently a
    "precision" profile concept (building a standard-form anchor map), so it
    always names a precision playbook_id rather than riding the ambient
    default."""
    failures = []
