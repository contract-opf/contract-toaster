...
standard-forms/synthetic-generic-v1.0.0.docx (issue #200), and re-verifies every
...
## What this test asserts
  ...
  2. protects.required_tokens (every on_remove_or_alter hard-rejection rule
     in the registered "synthetic-generic" playbook_id's playbook JSON) are verified against the REAL parsed
...
def main():
    ...
    section_config = bam.load_section_config("synthetic-generic")
    ...
    # -------------------------------------------------------------------
    violations = bam.verify_required_tokens_against_docx(SYNTHETIC_DOCX, playbook_id="synthetic-generic")
    if violations:
        failures.append(
