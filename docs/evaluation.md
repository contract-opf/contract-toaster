...
This document describes how `contract-toaster` is held to a quality bar: the gold test set that gates every model integration, the regression gates that gate every playbook and prompt change, and the human-review feedback loop that grows the gold set over time. It is the authoritative home for the evaluation harness. For the LLM design and the playbook governance it gates, see [ARCHITECTURE.md → LLM](../ARCHITECTURE.md#llm--bedrock--claude-reviewer-and-critic-passes) and [ARCHITECTURE.md → Redlining](../ARCHITECTURE.md#redlining--owned-docx-library). For the threats the harness is part of defending against, see [docs/threat-model.md](threat-model.md). For the foundation backlog, see [docs/phase-0-issues.md](phase-0-issues.md).
...
### Implementation status: the offline LLM-native harness (issue #400)

The four-check description above is the full regression-gate REGIME this document specifies — live model calls, retrieval, stochastic-stability runs, and anchor/hash-verified redline patching, all gated in CI. `scripts/eval_harness.py` is the piece of that regime that is actually built today, and it is deliberately narrower: an **offline, model-free-input harness** that drives the real composed pipeline (`scripts/review_spine.py::run_review` — extract → primary → critic → reconcile → leakage-scan → redline) with **canned model responses** (`FakeBedrockClient`, `backend/src/model_client.py`) instead of live Bedrock calls. Each fixture supplies a synthetic document and a canned primary/critic response pair; the harness checks the pipeline's output CONTRACT against the fixture's declared expectations — decision fidelity, issue-count bounds, every proposed edit actually reaching the document (issue #628: under the v3 block-transcript contract that means the result reports no `analysis_report.changes_not_applied` entries — the check the retired quote-locate assertion became), and a leakage-planted case actually blocking to `ERROR_MANUAL_REVIEW_REQUIRED`.

**What this proves**: the pipeline's MECHANICAL wiring is correct — given a particular model answer, the reconciliation/leakage/redline machinery downstream of it does the right thing. This is real regression coverage: a change that breaks reconciliation, the leakage gate, or the quote-based redline patcher fails this harness with no Bedrock spend at all.

**What this does NOT prove**: that a real model's legal judgment is good. The harness never asks "would a real Opus/Sonnet call have reached the right decision on this draft" — that question requires an actual model call, which is exactly the "Full stochastic gate" tier described above (§[Gate tiers](#gate-tiers)) and remains a separate, human-executed step, not something `scripts/eval_harness.py` runs in CI. Retrieval regression, stochastic-stability runs, and anchor/hash-verified redline correctness against a real `.docx` corpus are likewise out of this module's scope; they remain the aspirational full regime this document specifies, not yet wired to run automatically.
...
The deterministic hard-rejection detectors (see [ARCHITECTURE.md → Retrieval](../ARCHITECTURE.md#semantic-retrieval-plus-a-deterministic-lexical-layer) and [docs/playbook-governance.md](playbook-governance.md)) run over the standard-form **diff**, so they can be checked in isolation, deterministically, without invoking the model. Three checks gate `draft → active`. **Issue #380/#400 note:** the LLM-native review path no longer uses these detectors or the standard-form diff at all — they remain live ONLY for this deterministic playbook-authoring gate, run by `tests/lint-gold-fixtures.py` and `tests/lint-acceptable-variations.py` against `tests/gold-fixtures/` (not by `scripts/eval_harness.py`, which is the LLM-native harness described above).
...
> **Superseded by issue #618.** The manifest change this gate was built to police has been reversed: the critic now receives the document the primary read, because issue #380 retired the standard-form diff the omission depended on. The parity baseline recorded below therefore measures an input shape the critic no longer gets; establishing a baseline for the #618 shape is an open eval task, and until it exists this gate is history rather than a live bar.
...
| `reject-one-way-hold-harmless` | Single planted hard rejection | `no-exos-indemnity` (must fire) | One-way Company hold-harmless obligation ("the Company shall defend, indemnify, and hold harmless Institution"). D2: verifies `hold harmless` trigger fires and is not over-exempted. |
| `reject-one-way-indemnify` | Single planted hard rejection | `no-exos-indemnity` (must fire) | One-way uncapped Company indemnification with no `hold harmless`/`duty to defend` language ("the Company shall indemnify Institution … with no cap on liability"). D2: verifies the `company shall indemnif` regex trigger catches the bare-indemnify violation class that escaped detection before the fix. |
| `reject-one-way-will-indemnify` | Single planted hard rejection | `no-exos-indemnity` (must fire) | One-way Company indemnification using "will indemnify" phrasing — a variant that escaped the prior `company shall indemnif` trigger. D2: verifies the broadened `company\s+(shall\|will\|must\|agrees?\s+to)\b[^.]{0,40}indemnif` regex catches "will/must/agrees to indemnify" phrasings. |
