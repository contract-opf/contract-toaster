...
- [ ] **Dependency noted:** CODEOWNERS must point at real team handles for the above to actually gate (it currently uses placeholders) — track replacing the placeholder GC/engineering team slugs with real handles as a prerequisite to the control being live.
...
Files were drafted during the initial design phase. CODEOWNERS uses placeholder team slugs for the GC and engineering teams; replace these with real handles once teams are created in the org.
...
restricted to the `example.com` hosted domain. Domain membership alone is **not**
...
- [ ] Hosted-domain restriction is **defense-in-depth, enforced in BOTH layers** (not "pick one"): (a) the Google OAuth request pins `hd=example.com` **and** a Cognito pre-sign-up / pre-token-generation Lambda rejects any non-`@example.com` verified email; (b) the backend JWT validator independently re-verifies the `email` domain and the `hd` claim on every request. Backend enforcement is built in this issue's companion (App Runner) issue but the contract is documented here.
- [ ] **Authorization beyond the domain:** a verified `@example.com` email is necessary but **not sufficient**. Access additionally requires membership in the application allowlist — the authoritative gate is the **DynamoDB `users` allowlist row** (`active` status). The pre-token-generation Lambda denies sign-in (and the backend independently denies requests) for an in-domain user who is not allowlisted.
...
- [ ] CDK deploy succeeds in dev; manual smoke test confirms a `@example.com` allowlisted Google account can sign in, a `@example.com` account **not** on the allowlist cannot, and a non-`@example.com` account cannot.
...
Scaffold the React SPA and wire it to Amplify Hosting. The app at this stage shows a sign-in button and, after sign-in, "Signed in as you@example.com" plus the version number from the backend's authenticated `/version` endpoint. Nothing else.
...
- [ ] **JWT verification middleware is in scope here (security ⇒ Phase 0):** verifies the Cognito token signature, audience, and expiry, **and** independently re-verifies the `email` domain and Google `hd` claim are `example.com` (the backend half of the two-layer hosted-domain enforcement). A `/whoami` (or equivalent) authenticated echo endpoint proves it end-to-end.
...
- [ ] A `@example.com` Google account can sign in via Cognito.
- [ ] A non-`@example.com` Google account is rejected at sign-in with a clear message.
...
The review cannot soundly judge a draft without comparing it to the canonical the tenant standard
