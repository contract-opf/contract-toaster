...
`tools/document_spine_smoke.py` runs any corpus of `.docx` files through the model-free document spine — `scripts/extraction_normalization_stage.py` → `scripts/block_transcript.py` → `scripts/redline_block_apply.py` — with **no playbook, no model, no network, no API key**, and prints only per-document and aggregate counts, ratios, and reason codes.

**Repointed 2026-08-31 (issue #628).** The harness used to drive the quote locator and quote patcher; those modules are deleted, and the spine it probes is now the block-transcript path a real review actually takes. The harness itself, its two roles, and its privacy invariant are unchanged — only the middle two stages and the vocabulary of the counts they print.

It has two roles:

1. **A discovery instrument for a private corpus.** Point `CORPUS_DIR` at real counterparty paper (yours, never committed, never read by this repo's own tests) and read the printed ratios and reason codes to find structural failure classes the spine does not yet handle. This is exactly how issues #560/#561 (a reserved `ns0` namespace prefix crashing the redline) and #564 (an edit spanning a multi-`<w:p>` paragraph join) were found in the first place — by running the real pipeline against a real, private, 26–31 document corpus and measuring what broke.
...
2. **Derive transcripts.** For a normalized document, `extraction_normalization_stage.build_block_map()` gives the addressing view, and the harness derives one **whole-block replace** transcript per non-empty block: a `delete` of that block's entire own `text` followed by an `insert` of the same text plus a fixed, content-free literal suffix. No model call stands in for what a real model would actually write; because the `delete` transcribes the block character-for-character by construction, any rejection below is the *spine's*, never transcription noise a real model would have introduced.
3. **Prove** (`block_transcript.validate_block_patches`), once per block: `proven`, or one of that module's own rejection reasons (`unknown_block_id`, `source_mismatch`, `alignment_ambiguous`, `no_edit_segments`, `malformed_segments`, `duplicate_block_id`, `malformed_block_op`, `conflicting_block_op`). One call per block deliberately — the validator is all-or-nothing over the transcript it is given, so batching a whole document into one call would collapse it to a single reason and hide which blocks actually failed.
4. **Compile** (`redline_block_apply.apply_block_transcript`), once per document, batching every proven block as one transcript: `applied` count, plus a `failures` reason histogram (`spans_physical_paragraph` — issue #564's join, where the edit is genuinely addressed but crosses a physical-paragraph boundary the writer cannot write across; `block_text_changed`; `paragraph_not_resolved`; `edit_not_applied`; `round_trip_verification_failed`; and the rest of `redline_block_apply.FAILURE_REASONS`).
...
- **`refused` reason codes** — `classify_unnormalizable_reason()` maps a normalization failure's notes to one of a small, fixed set of symbolic codes (`accepted_change_missing_resulting_text`, `malformed_pending_revision`, `unknown_tracked_change_status`, `unrecognized_revision_type`, `suspicious_control_characters`, or `unclassified_unnormalizable` for anything that matches none of the above). `pending_change_inside_field_code` was retired by issue #530: a pending change inside a field code no longer fails closed, so that code can no longer occur. `malformed_pending_revision` and `accepted_change_missing_resulting_text` survive but are unreachable from a real `.docx` since issue #93: they now require the revision record's `resulting_text` KEY to be ABSENT, and `extraction_normalization_stage._build_paragraph_record` always sets it — a whole-paragraph deletion (`resulting_text == ""`) normalizes as a deletion instead of classifying here. A nonzero count for either against real documents means a caller other than the extractor produced the record. `suspicious_control_characters` was added by issue #632: extracted text carrying zero-width or bidirectional control characters fails closed rather than being stripped (see `scripts/normalize_input.py` → “Control-character screen”). The note it classifies is itself counts-only, so nothing about it widens what this tool prints.
...
- **`proven` ratio dropped / a rejection reason went up.** Either real documents got structurally weirder (a new revision-history pattern, a new punctuation encoding) or a change to `extraction_normalization_stage.py` / `scripts/text_fold.py` / `scripts/block_transcript.py` regressed the proof. This is a strong signal precisely *because* the derived transcript is the document's own text: a `source_mismatch` here means the block map and the block's real characters disagree, which no model behaviour can explain away. Compare against the previous run's numbers on the *same* corpus before concluding anything.
...
## `tools/churn_docx.py`: the known failure classes
...
| `curly_punctuation` | Word's typographic auto-correct (`'`/`"`/`-` → `'`/`"`/`–`) throughout the document | `scripts/text_fold.py`'s `TYPOGRAPHIC_FOLD` table — 15 of 16 documents in the real EIAA corpus carried curly punctuation |
...
| `content_control_wrapped_text` | A filled-in content control (`<w:sdt>`) wrapping text inline and a whole clause paragraph, plus a `<w:smartTag>` inline | Issue #94 — a filled-in `w:sdt`/`w:smartTag` is a transparent wrapper, same as `w:hyperlink`; only a GENUINE placeholder (`w:sdtPr/w:showingPlcHdr`) stays excluded |
