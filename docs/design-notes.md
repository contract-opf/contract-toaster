...
**Full-doc threshold on the primary side (not the critic).** The primary reviewer does benefit from full verbatim context on documents below `full_doc_token_threshold` (default 60,000 tokens, raised from 15,000 by issue #419 — the original default silently degraded any ~30-page-and-up MSA to an outline-only review with no signal anywhere in the result), because the primary must produce replacement text that fits the counterparty's actual draft — and having the full doc nearby helps. Above the threshold, a section outline (heading + word count) preserves navigational structure at a fraction of the token cost, and the degrade is no longer silent: the review result carries `input_mode="section_outline"`, `confidence_state` is degraded one level, and a fixed, substance-free sentence is appended to the summary (`scripts/reconciliation.py::reconcile`). The threshold is a config value, not a hard-coded constant, and is recorded per review so the eval harness can gate on it.
