...
We use Anthropic Claude (Opus 4.8 on the reviewer pass; under [ADR 0001](adr/0001-critic-has-the-last-word.md) the critic is pinned at or above the reviewer's tier) via Amazon Bedrock with retrieval-augmented generation over a legally curated corpus of the tenant agreements, plus a codified playbook embedded in the system prompt. We do not fine-tune. (The matrix is a deliberate pin chosen under the model-policy matrix, not "whatever is newest"; see "Why a model-policy matrix, not an automatic model choice" below.)
...
> **Retired 2026-09-02 (issue #631).** The deterministic detector engine and the standard-form diff it read were retired from issue generation by the 2026-07-22 LLM-native decision (D3), and issue #631 deleted the code (`scripts/diff_standard_form.py`, `scripts/build_anchor_map.py`, `scripts/redline_patch.py`, `scripts/redline_inplace.py`, `scripts/redline_docx_writer.py`, `scripts/form_match_router.py`, and the anchor/diff CI gates). Issue spotting is LLM-native over the document itself, and an edit is addressed by **block transcript** — see [ARCHITECTURE.md → How an edit is addressed: block transcripts](../ARCHITECTURE.md#how-an-edit-is-addressed-block-transcripts-issues-619628-rewritten-2026-08-31). The paragraph below is kept as the record of why the retired design read a diff rather than raw text; that reasoning is still why the LLM-native pass is given the document with its structure intact rather than a flattened blob.
...
- **A second pass that is the senior reviewer, not a cheaper flagger.** A single review pass misses things, so we run a second one. The original framing made that second pass a deliberately *different*, cheaper model (Sonnet against an Opus primary) for decorrelated blind spots, and gave it no authority over the words: it could flag a drifting replacement but never replace it. [ADR 0001](adr/0001-critic-has-the-last-word.md) reversed that. The critic is now the **senior reviewer**: given the playbook, the document, and the first pass's complete output, it reads the first pass with a sceptical eye and emits the final issues and the final replacement text. Its authority is *validated later-in-time* — it runs later and has strictly more information — so the policy requires it to be **at least as capable** as the reviewer rather than cheaper, and the default puts the stronger model on it. The pass with the most information should own the output it is best placed to improve; decorrelation bought by a weaker second opinion was paying for that with quality. Reconciliation stays deterministic: hard rejections and detector fires remain monotonic, every override is recorded with both versions, and the critic's text runs through every gate the reviewer's text does (see reconciliation below).

### Why the critic prompt omits the raw counterparty document

> **Superseded by issue #618.** The critic now *does* receive the counterparty document the primary pass read. Both arguments below rest on the standard-form diff carrying the "what changed" signal — and **issue #380 retired the diff**: `scripts/review_spine.py::run_review` passes `diff_hunks=[]` / `anchored_clauses=[]`, so what the leaner input actually left the critic with was the primary's JSON alone, with no way to check a `source_quote` against its source. This section is kept as the record of the original 2026-06-11 decision and of the efficacy claim that would have to be re-argued to reverse it back. For what ships, see [ARCHITECTURE.md → Per-pass prompt manifest](../ARCHITECTURE.md#per-pass-prompt-manifest).
...
**Document size: one quality, or a loud failure (owner decision, 2026-08-25, issue #625) — supersedes the full-doc threshold.** There is no longer a threshold at all. The primary reviewer is *always* shown the full verbatim document, because it must produce replacement text that fits the counterparty's actual draft, and `max_input_tokens` (raised to 100,000 by this decision) is the only size rule: at or under it the document is reviewed in full; above it the review terminates as `MANUAL_REVIEW_REQUIRED` / `document_too_large` before any model call.

Issue #419's `full_doc_token_threshold` and its section-outline fallback are **deleted**, along with the `input_mode` result field, the one-level confidence degrade, and the fixed summary notice that made the degrade visible. Those signals were the right mitigation for the wrong feature. The objection is not cost, it is correctness: a model must never redline text it did not receive, and a heading-and-word-count digest is not the document. Marking the result as low-confidence does not repair a redline drafted against text the model never saw — it only labels it. A document too large for the cap is a document that needs a human, and saying so loudly is a better answer than a quietly worse review. Batching or segmenting oversized documents is deliberately not in scope; failing closed is the whole design.

### Why the two passes reconcile by deterministic rule, not by a third model

The critic has the last word on the review, but it does not get to *arbitrate* the merge, and the two outputs are not blended by "vibes." We merge them with fixed, auditable semantics because a non-deterministic merge would reintroduce exactly the uncertainty the second pass exists to remove:
...
- **Every override is recorded.** The original add-only rule — the critic could surface new issues and challenge replacement text, but never substitute its own replacement language for the primary's — is **superseded by [ADR 0001](adr/0001-critic-has-the-last-word.md)**. The critic's replacement text is now the text that ships; what the merge guarantees instead is that no override is silent, because `critic_delta` records the primary's version, the critic's version and the critic's stated reason for every issue where the two differ. Nothing the primary said is lost from the record.
...
The tempting shortcut is "anyone with a `@example.com` Google account can use the tool." We rejected it. Domain membership answers *who you are*, not *whether you should be reviewing legal documents* — and the population of a corporate domain is large, churning, and includes contractors, interns, and every function in the company. For a tool that touches legal-facing content, "you have a company email" is not consent to access.
...
### Why `structured_outputs` in the OpenRouter policy means "on our ZDR endpoint," not "the model supports it"

`model-policy/openrouter.json`'s per-model `structured_outputs` field (issue #562) gates whether `backend/src/model_client.py` adds `response_format: {type: json_schema}` to a request. Issue #142 found the field had been set from the wrong fact: the 2026-08-02 verification pass confirmed each id advertised `response_format` *somewhere* in OpenRouter's catalogue, not that the specific endpoint reachable under this deployment's provider block — `zdr: true`, `data_collection: "deny"`, `require_parameters: true` (data-handling posture for counterparty documents, see [data-handling.md](data-handling.md); never weakened to chase a capability) — actually accepts it. For Claude Opus 5, Opus 4.8, and Sonnet 5, it doesn't: OpenRouter's routing funnel filters a `response_format` request for those ids to zero endpoints at the "Filter by Data Policy" step (neither endpoint offering `response_format` for those ids is zero-data-retention), which surfaced to the attorney as every Opus 5 review dying with an HTTP 404 before the first token.

So the field's meaning is now precisely: *a request for this model_id carrying `response_format`, sent through this deployment's exact provider block, does not 404.* Absence still means False (fail-closed — never an assumed capability the file did not measure), and a `false` value for one of these ids does not mean the model can't produce schema-shaped output at all — the forced-tool path (`OPENROUTER_STRUCTURED_OUTPUT=1`, the default; `tools`/`tool_choice` rather than `response_format`) is independent of this field and routed fine in the same probe, so schema enforcement for Opus 5 still runs, just not via `response_format`.

**Re-probe procedure**, unchanged in shape from the original pass but now run against the production provider block instead of a bare catalogue query: for each candidate `model_id`, send one request with `provider` set to exactly the block above and `response_format.type=json_schema` set (a plain request or a `tools`/`tool_choice` request is not equivalent evidence — both routed fine for every id checked on 2026-09-16; only `response_format` 404'd for the three Anthropic ids above). A 200 means the field may be `true`; a 404 whose body's `metadata.routing_funnel` shows `failed_routing_step: "Filter by Data Policy"` means it must be `false`. Record the probe date and result in a `structured_outputs_note` (role pins) or in the file's `_comment` (`selectable` entries) — see `model-policy/openrouter.json`'s `_comment` for the full record of the 2026-09-16 pass. Do **not** record it in a `selectable` entry's `note`: that field is operator-facing copy, serialised whole to `GET /api/admin/model-selection` and rendered inside each `<option>` label of the admin model picker, so it stays one short line. The four non-Anthropic `selectable` entries (GPT-5.6 Sol, Gemini 3.1 Pro, Kimi K3, DeepSeek V4 Pro) were not re-probed by issue #142 — its live probe covered only the Anthropic ids this deployment pins — so their `true` values still rest on the original catalogue-advertised pass and have not been checked against this deployment's own ZDR provider block. That un-probed status is recorded in the policy file's `_comment` (the STILL EXPOSED paragraph) and in the paragraph below, not in those entries' `note` fields — those are admin-picker copy.

**What is still exposed, and where.** One of those four is not merely selectable: `deepseek/deepseek-v4-pro` is also the `models.preflight` role pin, and `backend/src/review_routes.py`'s `POST /api/reviews/preflight` invokes it with `output_schema=` and no `tool_spec=` — the only shipping call site with no forced-tool fallback. Because that entry still declares `structured_outputs: true`, `model_client.py` still attaches `response_format` to every preflight request, through the same ZDR provider block that 404'd Opus 5, for an id issue #142 did not re-probe: the identical failure may be live on that route today. It would not be noticed — the call sits inside `except Exception:  # degrade to stats-only`, so a 404 silently drops the upload-time classification guess rather than surfacing as a failure. Re-probing `deepseek/deepseek-v4-pro` is the highest-value next step; note that flipping it to `false` is not by itself sufficient, since the preflight path has no forced-tool fallback to fall back to and would need one first.
...
1. **The review pipeline is structure-first, not text-extraction-first.** (As written in 2026-06 this said *diff*-first: the primary input was a deterministic diff against the canonical standard form. Issue #631 deleted that diff — see the retirement note above — but the conclusion is unchanged and if anything stronger, because the block-transcript addressing that replaced it depends on the uploaded package's own OOXML structure even more directly than the diff did.) The pipeline works against an OOXML representation: the normalization pass, the block map an edit is addressed against, and the redline writer all operate on the `.docx` XML structure. A PDF is a flat rendered surface — it carries no structural markup that maps to the anchored standard-form sections. Even perfect PDF text extraction produces a block of text with no reliable section mapping; the diff would be meaningless (whole-document, with no clause anchors) and the redline would have nowhere to patch.
...
So we keep a separate **internal confidence / error state** that is a *system status*, not a legal category. It is shown to the reviewer as a `confidence_band` before the download, so "the tool is less sure about this one" is disclosed without being laundered into a legal conclusion — which is operationally distinct from "the tool reviewed this and found nothing." This keeps the legal-facing decision honestly binary. An earlier draft of this section said low confidence with no concrete playbook issue *routed the review to manual review*; the owner retired that projection on issue #96 (Option B, 2026-09-16): a review that completes is `DONE` whatever its band, and `MANUAL_REVIEW_REQUIRED` is reserved for a run that did not complete at all. The state machine that carries this status is owned by [ARCHITECTURE.md](../ARCHITECTURE.md).
...
Approval is a human act with legal consequence. This tool produces drafts and analysis. The signed agreement that goes back to the school is reviewed and approved by a human attorney through the tenant's existing approval channel. The tool's audit log captures what the tool said; the human's approval lives elsewhere.
...
- Word add-in. the tenant already has one.
- Approval workflow.
