...
| Manual-review filter / disposition capture | `RUNBOOK.md:615-617` | **SHIPPED** (capture) / **STUBBED** (filter) | Capture is **SHIPPED** (issue #486): `POST /api/reviews/{review_id}/disposition` wires `backend/src/disposition.py`'s `record_disposition` behind a real, owner-scoped, tested route, and both `ReviewSubmission.tsx`'s finished-review panel and `ReviewHistory.tsx`'s Disposition column let a reviewer record and see it end-to-end. The cross-user manual-review-filter / legal-triage-queue half (`list_legal_triage_queue` / `mark_triaged`) is still **STUBBED** — no API endpoint or admin UI screen surfaces it; deferred to #91/#252 per #486's own "Defer" section. |
