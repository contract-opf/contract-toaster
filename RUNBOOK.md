...
5. **Set stack parameters.** Copy `infra/cdk.context.example.json` to `infra/cdk.context.json` and fill in `githubOwner`, `alarmsEmail`, `appDomain`, `adminEmail` (the GC's `@company.com` address), and `hostedDomain` — see `infra/README.md` "Deploy identity context" for what each controls. `infra/cdk.context.json` is **git-ignored**; it holds this deployment's real tenant values and is never committed (issue #409).
...
7. **Know which tables the stack owns, and what happens when one is missing.** The tables `infra/lib/nested/data-stack.ts` declares are created by CDK — including the entity-roster table `contract-toaster-entity-roster-<env>`, whose name App Runner receives as `ENTITY_ROSTER_TABLE`. The API **creates none of them at runtime** (issue #59, audit finding F9): until #59, `backend/src/entity_roster.py` issued a `create_table` on every roster read and swallowed the "already exists" error, which put a write-class call on a read path and would have produced a table with no CMK, no PITR and no removal policy had the role ever been allowed to make one. Instead `backend/src/startup_checks.py::ensure_entity_roster_table` runs one `describe_table` in the ASGI lifespan, and on this AWS target a missing table **refuses the boot** with:

    ```
    STARTUP: refusing to start — the entity-roster table named by ENTITY_ROSTER_TABLE does not exist. …
    ```

    That is a restart loop, which is the intended outcome: deploy the stack (or point `ENTITY_ROSTER_TABLE` at the table it created) rather than restarting the service and hoping. Only a genuinely absent table is fatal — a throttle or a `DescribeTable` permission gap logs a warning and starts, because the roster degrades to the playbook's own `perspective.party` by design. On the **Docker Compose target** the same helper creates the table at startup if `deploy/dts/bootstrap.py` has not already; see [deploy/dts/README.md](deploy/dts/README.md) → "Our own legal entities".

8. **Verify health.** Visit the App Runner URL `/health` (public) and confirm liveness. For the deployed version, commit SHA, and serving **image digest**, call the **allowlisted `/version`** endpoint with a valid token (build details are not exposed unauthenticated). App Runner deployment metadata also shows the serving digest.

9. **Verify Cognito.** Visit the Amplify URL, sign in with a `@company.com` Google account, confirm successful sign-in and that a row exists in DynamoDB `users`.

10. **Install the first playbook.** The image ships with one brand-free sample playbook, `synthetic-nda-sample` ("Synthetic NDA Sample"). **On this AWS path it is NOT installed for you** — the seed that installs it (`backend/src/sample_playbooks.py::seed_shipped_playbook`) runs only from the Docker Compose deploy bootstrap (`deploy/dts/bootstrap.py`, the container `deploy/dts/docker-compose.yml` starts), which App Runner does not run. Here, install a playbook the ordinary way: upload a version and activate it through the admin playbook routes. There is no bespoke one-click sample activation any more (issue #433 retired it), so nothing about the shipped sample is a special case — it installs and activates through the same `record_playbook_version_upload` / `activate_playbook_version` functions as anything else. Verify either way with `GET /api/playbooks` (the playbook you installed should read `active`); until something is active, submissions are correctly refused with `503 no active playbook`. To run your own tenant playbook, upload your playbook JSON (authored per `playbooks/schema.json`) as a **draft** version through the normal admin UI and activate the release bundle: playbook hash, prompt hash, canonical standard-form hash, anchor-map hash, model-policy hash, corpus snapshot, evaluation run ID, and Legal approval. (Once we have a corpus, seed it as a draft corpus snapshot and activate only after curation and retrieval regression checks pass.)

11. **Verify alarm delivery (bootstrap acceptance — required before declaring prod operable).** An unconfirmed SNS email subscription receives no alarm mail — silent monitoring failure is the exact defect this step closes. Two sub-steps, both required:
...
**What a ship-ready document actually contains, per notes mode (issue #524, epic #519).** The
marker answers "is this copy for us?" — it does not answer "what is in it?". A review's **notes
mode** decides both, and it is chosen per review on the Review tab (issue #523):

| Notes mode | Footnotes in the delivered `.docx` | Export marker | Carries content written for us only? |
|---|---|---|---|
| `none` | none at all — bare tracked changes (`word/footnotes.xml` is omitted, not emitted empty) | absent | No |
| `external` | the counterparty-facing rationale per applied change, unmarked | absent | No |
| `internal` | your own team's note only, behind `[INTERNAL] NOT FOR THE COUNTERPARTY:` | present | **Yes** |
| `both` | both notes against the same change, external first — only the internal one marked | present | **Yes** |

The last two rows are the documents that are not the version to send externally; the first two were
never given internal-audience content to hold. (What the tool never does is bless the sending: the
substance of any redline is still yours to review before it leaves — nothing here enforces,
requires, or records approval.) `internal` and `both` are the two modes that put your own reasoning
in the file, and they are exactly the two that carry the marker. Both are unselectable
in production while issue #572's `NOTES_MODE_ENABLED` kill switch is off, so every review run today
lands in the top two rows and produces a document with no marker in any part. Third-party
counterparty paper renders no internal footnote at all today — see
[docs/output-contract.md → Which rationale becomes a footnote](docs/output-contract.md#which-rationale-becomes-a-footnote-the-reviews-notes-mode-issue-522-epic-519-item-d)
for that path's per-mode behaviour.

**Accepting all tracked changes does not strip the footnotes — it promotes them.** Each footnote
reference is emitted inside the change's `<w:ins>` run while the footnote body is ordinary
untracked text, so accepting all changes turns a footnote into plain body text rather than removing
it. That is deliberate (a footnote that vanished on accept-all would take the counterparty-facing
rationale with it), and it is why **"accept all changes and send it" is not a way to clean an
internal-notes document**: in an accept-all-then-send workflow the `[INTERNAL]` marking in the
footnote text is the last thing standing between your note and the counterparty. There is no
clean-copy conversion step to run; the clean copy is a review run in `none` or `external` in the
first place.

**The download filename is not a signpost.** A redline downloads as `<original name>-redline.docx`
in every notes mode (`backend/src/download.py::redline_filename_for`) — the filename does not say
whether the document carries internal notes, and epic #519's signposted-filename idea did not ship
(`frontend/src/notesMode.ts` records the same thing for the submission UI). The in-document marker
and the review's own notes mode are the only signals; a file sitting in a downloads folder tells
...
Bootstrap step 11 (see "Day-one bootstrap" above) is the **required** one-time proof that alarm delivery is live. It must be completed — and its outcome recorded — before the environment is declared prod-operable. The test covers two failure modes that configuration alone cannot rule out:
