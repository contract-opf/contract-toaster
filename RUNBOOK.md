...
9. **Seed the first playbook.** The image ships with one bundled, brand-free sample playbook, `synthetic-nda-sample` ("Synthetic NDA Sample"), that activates with one click: as an admin, call `POST /api/admin/playbooks/synthetic-nda-sample/activate-sample` (or use the equivalent admin-UI affordance). This is the low-ceremony first-run path (`backend/src/sample_playbooks.py`) — no upload, no Legal approval, nothing to author. To run your own tenant playbook instead, upload your playbook JSON (authored per `playbooks/schema.json`) as a **draft** version through the normal admin UI and activate the release bundle: playbook hash, prompt hash, canonical standard-form hash, anchor-map hash, model-policy hash, corpus snapshot, evaluation run ID, and Legal approval. (Once we have a corpus, seed it as a draft corpus snapshot and activate only after curation and retrieval regression checks pass.)
